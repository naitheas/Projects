...
router.get('/', async (req, res, next) => {
  try {
    const industries = await db.query(
      ...
      AS industry_code, industry, company_code 
      FROM industries 
      LEFT JOIN companies_industries 
      AS ci 
      ON industries.code = ci.industry_code
      LEFT JOIN companies 
      ON ci.company_code = companies.code`
    );
    const industry = industries.rows;
...
router.post('/', async (req, res, next) => {
  ...
  try {
    if (!industry) {
      throw new ExpressError('Request must an Industry Name', 422);
    };
